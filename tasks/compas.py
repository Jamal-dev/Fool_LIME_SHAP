...
from utils.dataset_preprocessor import preprocess_compas
...
def parameter_parser():
  ...
  """
  description = ["Main function will run on COMPAS: https://github.com/propublica/compas-analysis/ dataset",
...
def run_compas():
  ...
  # Define the biased feature
  biased_feature_name = 'race'
  ...
  xtrain,ytrain,xtest,ytest,feature_names=load_transfomed_data(biased_feature_name=biased_feature_name
                                                              , preprocess_fun=preprocess_compas
                                                              , unrelated_columns=2
                                                              , test_size=0.1
                                                              , params = params)
  ...
  categorical_features = [feature_names.index('unrelated_column_one'),
                              ...
                              feature_names.index('c_charge_degree_F'), 
                              feature_names.index('c_charge_degree_M'), 
                              feature_names.index('two_year_recid'), 
                              feature_names.index('race'), 
                              feature_names.index("sex_Male"), 
                              feature_names.index("sex_Female")]
  ...
  return(xtrain,ytrain,xtest,ytest,feature_names,categorical_features,biased_feature_idx,unrelated_feature_1,unrelated_feature_2,biased_feature_name,dataset_name)

# Lime experiment
def lime_exp(analysis_mode=False):
  """
  Run through experiments for LIME on COMPAS using 2 unrelated features.
  ...
  * We print out the rate at which features occur in the top three features
  ...
  if analysis_mode==False:
    lime_model_1 = lime_experiment(dataset_name='COMPAS', 
    ...
    lime_model_2 = lime_experiment(dataset_name='COMPAS', 
    ...
    lime_model_1 = lime_experiment_analysis(dataset_name='COMPAS', 
                    number_unrelated_column=1, 
                    categorical_features=categorical_features, 
                    feature_names=feature_names,
                    xtrain=xtrain,
    ...
    # Repeat the same thing for two features
    lime_model_2 = lime_experiment_analysis(dataset_name='COMPAS', 
                    number_unrelated_column=2, 
                    categorical_features=categorical_features, 
                    feature_names=feature_names,
                    xtrain=xtrain,
                    ...
                    racist_model=racist_model_f(biased_feature_idx),
                    innocuous_model=innocuous_model_psi_two(unrelated_feature_1,unrelated_feature_2))
  return lime_model_1, lime_model_2

# Shap experiment
def shap_exp(analysis_mode=False):
  """
  Run through experiments for SHAP on COMPAS using 2 unrelated features.
  * This may take some time given that we iterate through every point in the test set
  * We print out the rate at which features occur in the top three features
  ...
  if analysis_mode==False:
    shap_model_1 = shap_experiment(dataset_name='COMPAS', 
                    number_unrelated_column=1, 
                    feature_names=feature_names,
                    xtrain=xtrain,
                    ytrain=ytrain,
    ...
    shap_model_2 = shap_experiment(dataset_name='COMPAS', 
                    number_unrelated_column=2, 
                    feature_names=feature_names,
                    xtrain=xtrain,
                    ytrain=ytrain,
    ...
    shap_model_1 = shap_experiment_analysis(dataset_name='COMPAS', 
                    number_unrelated_column=1, 
                    feature_names=feature_names,
                    xtrain=xtrain,
                    ytrain=ytrain,
    ...
    # runing again for two unrelated columns
    shap_model_2 = shap_experiment_analysis(dataset_name='COMPAS', 
                    number_unrelated_column=2, 
                    feature_names=feature_names,
                    xtrain=xtrain,
                    ytrain=ytrain,
                    ...
                    racist_model=racist_model_f(biased_feature_idx),
                    innocuous_model=innocuous_model_psi_two(unrelated_feature_1,unrelated_feature_2))
  return shap_model_1, shap_model_2


def compas_exp(analysis_mode=False):
  """
  Run through experiments for both LIME and SHAP on COMPAS using 2 unrelated features.
  * This may take some time given that we iterate through every point in the test set
  * We print out the rate at which features occur in the top three features
  ...
  xtrain,ytrain,xtest,ytest,feature_names,categorical_features,biased_feature_idx,unrelated_feature_1,unrelated_feature_2, biased_feature_name, dataset_name = run_compas()
